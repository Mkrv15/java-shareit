...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto updateItem(Long userId, Long itemId, ItemDto itemDto) {
        ...
        if (targetItem.getUserId() != user.getId()) {
            throw new NotFoundException(String.format("У пользователя с id %s не найдена вещь с id %s",
                    userId, itemId));
        }
        ...
        if (item.getRequestId() != null) {
            targetItem.setRequestId(item.getRequestId());
        }
    ...
    public List<ItemDto> getAllItems(Long userId, Integer from, Integer size) {
        ...
        List<Item> items = itemRepository.findAllByUserIdOrderById(user.getId(), pageable);
    ...
    public List<ItemDto> searchItems(String text, Integer from, Integer size) {
        ...
        if (text.isBlank()) {
            ...
            items = itemRepository.findByNameOrDescriptionLike(text.toLowerCase(), pageable);
        }
        return items
                .stream()
                .map(itemMapper::convertToDto)
    ...
    public void removeItem(Long userId, Long itemId) {
        ...
        Item item = itemRepository.findById(itemId).orElseThrow(() ->
                new NotFoundException(String.format("Вещь с id %s не найдена", itemId)));
        if (item.getUserId() != userId) {
            throw new NotFoundException(String.format("У пользователя с id %s не найдена вещь с id %s", userId, itemId));
        }
    ...
    public CommentDto addComment(Long userId, Long itemId, CommentDto commentDto) {
        ...
        List<Booking> bookings = bookingRepository.findAllByItemIdAndBookerIdAndStatus(
                        itemId, userId, Status.APPROVED, Sort.by(Sort.Direction.DESC, "start"))
        ...
        if (!hasCompletedBooking) {
            throw new CommentAccessDeniedException(
                    String.format("Пользователь с id %d не может оставлять комментарии вещи с id %d, так как бронирование еще не завершено.",
                            userId, itemId));
        }
