...
public class BookingServiceImpl implements BookingService {
    ...
    private final BookingStrategyFactory bookerStrategyFactory;
    private final OwnerBookingStrategyFactory ownerStrategyFactory;
    ...
    public BookingDto addBooking(long bookerId, BookingInputDto bookingInputDto) {
        ...
        Item item = itemRepository.findById(booking.getItem().getId()).orElseThrow(() ->
                new NotFoundException(String.format("Вещь с id %s не найдена", booking.getItem().getId())));
    ...
    public BookingDto approveOrRejectBooking(long ownerId, long bookingId, boolean approved, AccessLevel accessLevel) {
        Booking booking = getBookingById(bookingId, ownerId, accessLevel);
        if (booking.getStatus().equals(Status.APPROVED)) {
            throw new ValidationException(String.format("У бронирования с id %d уже стоит статус %s",
    ...
    public Booking getBookingById(long bookingId, long userId, AccessLevel accessLevel) {
        Booking booking = bookingRepository.findById(bookingId).orElseThrow(
                () -> new ValidationException(String.format("Бронирование с id %d не найдено", bookingId)));
        if (isUnableToAccess(userId, booking, accessLevel)) {
    ...
    public List<BookingDto> getBookingsOfCurrentUser(State state, long bookerId, Integer from, Integer size) {
        ...
        List<Booking> bookings = strategy.findBookings(booker.getId(), pageable);
    ...
    public List<BookingDto> getBookingsOfOwner(State state, long ownerId, Integer from, Integer size) {
        ...
        List<Booking> bookings = strategy.findBookings(owner.getId(), pageable);
    ...
    private void validateAddBooking(long bookerId, Booking booking, Item item) {
        if (bookerId == item.getUserId()) {
            ...
            throw new ItemNotAvailableForBookingException(String.format("Вещь с id %d не доступна для бронирования.",
                    item.getId()));
        } else if (isNotValidDate(booking.getStart(), booking.getEnd())) {
            throw new ValidationException("Даты бронирования выбраны некорректно.");
        }
    }
}
